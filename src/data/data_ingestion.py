...
def read_dask_df(
        data_path:Path,parse_dates:list=["tpep_pickup_datetime"],
        columns: list =[
            'trip_distance', 
...
if __name__ == "__main__":
    ...
    # real all the dataframes
    dfs = []
    #loop and read all the dataframes
    for df_name in df_names:
        df_path = raw_data_dir/ df_name
        df = read_dask_df(df_path)
        dfs.append(df)
        logger.info(f"Dask DataFrames are read successfully: {df_name}")

    #conncate all dfs
    df_final =dd.concat(dfs,axis =0)
    logger.info("All datasets merged successfully")

    # execute dask pipeline
